...
export default function PartnerDetail(props: any) {
	...
	const { data: employeeData, isLoading, refetch } = useGetUserEmployeeWithIdQuery(uid);
	...
	return (
		<View style={styles.container}>
			<Header title={t('Partner Details')} leftButton={() => props.navigation.goBack()} />
				...
				<ScrollView>
					...
					<View style={styles.salesDataPointsContainer}>
						<View style={styles.salesDataPointItem}>
							<Text style={styles.salesDataPointItemLabel}>{t('Paid Sales')}</Text>
						...
						<View style={styles.salesDataPointItem}>
							<Text style={styles.salesDataPointItemLabel}>{t('Unpaid Sales')}</Text>
							<Text
								style={[
					...
					<View style={{ width: '90%', alignSelf: 'center', marginVertical: 5 }}>
						<Text style={{ fontWeight: '500', fontSize: 20 }}>{t('Sales')}</Text>
					</View>
					<View>
